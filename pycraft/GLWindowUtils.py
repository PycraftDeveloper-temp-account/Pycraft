if __name__ != "__main__":
    ...
    class BenchmarkWindow(mglw.WindowConfig):
        def __init__(self, **kwargs):
            ...
            self.camera = self.SharedData.mod_ModernGL_window_keyboard_camera(
                self.wnd.keys,
        ...
        def key_event(self, key, action, modifiers):
            try:
                ...
                try:
                    self.wnd.close()
                    self.SharedData.error_message = "".join(("GLWindowUtils > BenchmarkWindow ",
        ...
        def close(self):
            try:
                ...
                self.SharedData.error_message = "GLWindowUtils > BenchmarkWindow > close: " + \
                    str(Message)
    ...
    class CameraWindow(mglw.WindowConfig):
            ...
            try:
                ...
                if action == keys.ACTION_PRESS:
                    if key == keys.L:
                        ...
                        self.SharedData.mod_tkinter_utils__.TkinterInfo.CreateTkinterWindow(
                            self.SharedData)
                ...
                try:
                    self.wnd.close()
                    self.SharedData.error_message = "".join(("GLWindowUtils > CameraWindow ",
                                                            f"> key_event: {str(Message)}"))
        ...
        def resize(self, width: int, height: int):
            try:
                ...
                self.SharedData.real_window_width, self.SharedData.real_window_height = self.window_size
        ...
        def close(self):
            try:
                ...
                self.SharedData.error_message = "GLWindowUtils > CameraWindow > close: "+str(Message)
