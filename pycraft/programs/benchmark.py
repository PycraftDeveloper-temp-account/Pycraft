if __name__ != "__main__":
    ...
    class generate_benchmark:
        """
        This class does the bulk of the rendering for the benchmark start and results
        section and also manages the running and execution of the benchmark.
        ...
        def benchmark_gui(self):
            """
            This subroutine does the bulk of the rendering for the benchmark start and results
            section and also manages the running and execution of the benchmark.
            ...
            try:
                ...
                pygame.display.set_caption(f"Pycraft: v{self.version}: Benchmark")
                ...
                benchmark_font = self.subtitle_font.render(
                    "Benchmark",
                    self.aa,
                    self.secondary_font_color)

                fps_information_text = self.large_content_font.render(
                    "fps benchmark results",
                    self.aa,
                    self.font_color)
                fps_information_text_width = fps_information_text.get_width()

                file_information_text = self.large_content_font.render(
                    "Read test results",
                    self.aa,
                    self.font_color)
                file_information_text_width = file_information_text.get_width()

                hardware_information_text = self.large_content_font.render(
                    "Hardware results",
                    self.aa,
                    self.font_color)
                hardware_information_text_width = hardware_information_text.get_width()

                sixty_fps_data = self.small_content_font.render(
                    "60 Hz",
                    self.aa,
                    self.accent_color)

                one_four_four_fps_data = self.small_content_font.render(
                    "144 Hz",
                    self.aa,
                    self.accent_color)

                two_forty_fps_data = self.small_content_font.render(
                    "240 Hz",
                    self.aa,
                    self.accent_color)
                ...
                while True:
                    ...
                    if stage == 1:
                        ...
                        CPUhwINFO = self.small_content_font.render(
                            CPUid,
                            self.aa,
                            self.font_color)
                        CPUhwINFOwidth = CPUhwINFO.get_width()

                        RAMhwINFO = self.small_content_font.render(
                            RAMid,
                            self.aa,
                            self.font_color)
                    ...
                    if stage == 2:
                        try:
                            fps_Results = extended_benchmark.Loadbenchmark.run(self)
                    ...
                    if stage == 3:
                        ...
                        multiplier = len(fpslistX)/(self.real_window_width-20)
                        Results1 = []
                        for i in range(len(fpslistY)):
                            Results1.append([(fpslistX[i]/multiplier), fpsListY[i]])

                        multiplier = len(fpslistX2)/(self.real_window_width-20)
                        Results2 = []
                        for i in range(len(fpslistY2)):
                            Results2.append([(fpslistX2[i]/multiplier), fpsListY2[i]])

                        multiplier = len(fpslistX3)/(self.real_window_width-20)
                        Results3 = []
                        for i in range(len(fpslistY3)):
                    ...
                    if stage == 4:
                        ...
                        GreenInfo = self.small_content_font.render(
                            GreenResults,
                            self.aa,
                            self.font_color)

                        BlueInfo = self.small_content_font.render(
                            BlueResults,
                            self.aa,
                            self.font_color)

                        RedInfo = self.small_content_font.render(
                            RedResults,
                            self.aa,
                            self.font_color)
