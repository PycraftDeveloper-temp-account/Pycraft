if __name__ != "__main__":
    print("Started <Pycraft_display_utils>")

    class display_functionality:
        def __init__(self):
            pass

        def core_display_functions(self, location="Home", checkEvents=True, resize=True):
            if self.use_mouse_input is False:
                if (self.command != "Home" and
                        self.command != "Settings" and
                        self.command != "Undefined" and
    ...
    class display_utils:
        ...
        def update_display(self):
            ...
            try:
                try:
                    ...
                    print("display_utils > display_utils > update_display: "+ str(Message))
                    self.fullscreen = True
                    self.saved_window_width = 1280
                    self.saved_window_height = 720
        ...
        def set_display(self):
            ...
            try:
                try:
                    ...
                    print("display_utils > display_utils > set_display:", Message)
                    self.saved_window_width = 1280
                    self.saved_window_height = 720
                    self.mod_pygame__.display.quit()
                    self.mod_pygame__.init()
        ...
        def get_play_status(self):
            try:
                ...
                self.error_message = "display_utils > display_utils > get_play_status: "+str(Message)
                self.error_message_detailed = "".join(
                    self.mod_traceback__.format_exception(
                        None,
                        Message,
