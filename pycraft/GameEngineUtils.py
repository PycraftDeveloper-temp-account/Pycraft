if __name__ != "__main__":
    ...
    class ComputeWeather:
        ...
        def ComputeCloudModel(self, size):
            try:
                ...
                self.SharedData.error_message = "".join(("GameEngineUtils > ComputeWeather ",
        ...
        def generate_perlin_noise_2d(self, shape, res):
            ...
            grid = self.SharedData.mod_numpy__.mgrid[0:res[0]:delta[0],0:res[1]:delta[1]].transpose(1, 2, 0) % 1
            # Gradients
            angles = 2*self.SharedData.mod_numpy__.pi*self.SharedData.mod_numpy__.random.rand(res[0]+1, res[1]+1)
            gradients = self.SharedData.mod_numpy__.dstack((self.SharedData.mod_numpy__.cos(angles), self.SharedData.mod_numpy__.sin(angles)))
            g00 = gradients[0:-1,0:-1].repeat(d[0], 0).repeat(d[1], 1)
            g10 = gradients[1:,0:-1].repeat(d[0], 0).repeat(d[1], 1)
            g01 = gradients[0:-1,1:].repeat(d[0], 0).repeat(d[1], 1)
            g11 = gradients[1:,1:].repeat(d[0], 0).repeat(d[1], 1)
            # Ramps
            n00 = self.SharedData.mod_numpy__.sum(grid * g00, 2)
            n10 = self.SharedData.mod_numpy__.sum(self.SharedData.mod_numpy__.dstack((grid[:,:,0]-1, grid[:,:,1])) * g10, 2)
            n01 = self.SharedData.mod_numpy__.sum(self.SharedData.mod_numpy__.dstack((grid[:,:,0], grid[:,:,1]-1)) * g01, 2)
            n11 = self.SharedData.mod_numpy__.sum(self.SharedData.mod_numpy__.dstack((grid[:,:,0]-1, grid[:,:,1]-1)) * g11, 2)
            # Interpolation
            t = f(grid)
            n0 = n00*(1-t[:,:,0]) + t[:,:,0]*n10
            n1 = n01*(1-t[:,:,0]) + t[:,:,0]*n11
            return self.SharedData.mod_numpy__.sqrt(2)*((1-t[:,:,1])*n0 + t[:,:,1]*n1)
                ...
                if self.SharedData.mod_random__.randint(0, 100) <= 65:
                    ...
                    if self.SharedData.mod_random__.randint(0, 100) <= 80:
                        ...
                        if self.Time_Percent > 50: # night
                            self.skysphere_prog_Transparency = self.SharedData.mod_random__.randint(35, 50)/100
                        else: # day
                            self.skysphere_prog_Transparency = self.SharedData.mod_random__.randint(25, 40)/100
                        ...
                        if self.SharedData.mod_random__.randint(0, 100) <= 50:
                            ...
                            self.skysphere_prog_Transparency = self.SharedData.mod_random__.randint(0, 25)/100
                            self.skysphere_prog["transparency"] = self.skysphere_prog_Transparency
                            ...
                            self.skysphere_prog_Transparency = self.SharedData.mod_random__.randint(10, 45)/100
                            self.skysphere_prog["transparency"] = self.skysphere_prog_Transparency
            except Exception as Message:
                self.SharedData.error_message = "".join(("GameEngineUtils > ComputeWeather ",
        ...
        def AccessGUI(self):
            try:
                ...
                if self.Inventory:
                    ...
                    self.SharedData.mod_map_GUI__.GenerateMapGUI.MapGUI(self.SharedData)
                    self.Map = False
                ...
                self.SharedData.error_message = "".join(("GameEngineUtils > AccessOtherGUIs ",
                ...
                self.SharedData.mod_error_utils__.GenerateErrorScreen.ErrorScreen(self.SharedData)
    ...
    class ShadowmappingMathematics:
        ...
        def ComputeShadows(self):
            try:
                ...
                print(Message)
                self.SharedData.error_message = "GameEngine > GameEngine > __init__: "+str(Message)
                self.SharedData.error_message_detailed = "".join(
                    self.SharedData.mod_traceback__.format_exception(
                        None,
                        Message,
    ...
    class OnscreenEventFunction:
        ...
        def game_events(self):
            try:
                if self.SharedData.use_mouse_input is False:
                    self.camera.rot_state(
                        -self.Joystick_Rotation[0]*self.SharedData.camera_angle_speed,
                        -self.Joystick_Rotation[1]*self.SharedData.camera_angle_speed)
                    ...
                    if self.Wkeydown:
                        ...
                        if self.Sprinting:
                            if WPressedTime >= (self.SharedData.mod_random__.randint(25, 75)/100):
                                self.SharedData.mod_sound_utils__.PlaySound.PlayFootstepsSound(
                                    self.SharedData)
